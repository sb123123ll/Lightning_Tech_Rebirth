...
public record FrequencyResponsePacket(int responseCode) {
   ...
   private Component toMessage() {
      return switch (this.responseCode) {
         case 1 -> Component.m_237115_("ae2lt.gui.error.require_password");
         case 2 -> Component.m_237115_("ae2lt.gui.error.no_permission");
         case 3 -> Component.m_237115_("ae2lt.gui.error.invalid_frequency");
         default -> Component.m_237115_("ae2lt.gui.error.rejected");
         case 5 -> Component.m_237115_("ae2lt.gui.error.frequency_in_use");
      };
   }

   public static void handle(FrequencyResponsePacket pkt, Supplier<Context> ctxSupplier) {
      Context ctx = ctxSupplier.get();
      Component message = pkt.toMessage();
      ctx.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> ClientNetworkPacketHandlers.handleFrequencyResponse(message)));
