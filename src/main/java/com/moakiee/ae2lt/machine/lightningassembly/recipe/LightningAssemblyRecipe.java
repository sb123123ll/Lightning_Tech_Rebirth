...
public final class LightningAssemblyRecipe implements Recipe<LightningAssemblyRecipeInput> {
      ...
      if (!slotStacks.isEmpty() && slotStacks.size() <= 9) {
         ...
         for (LightningSimulationIngredient requirement : this.inputs) {
            ...
            for (int slotIndex = 0; slotIndex < slotStacks.size(); slotIndex++) {
               LightningAssemblyRecipeInput.SlotStack slotStack = slotStacks.get(slotIndex);
         ...
         List<LightningAssemblyRecipe.RequirementState> requirements = new ArrayList<>(this.inputs.size());
         ...
         for (int requirementIndex = 0; requirementIndex < this.inputs.size(); requirementIndex++) {
            ...
            requirements.add(new LightningAssemblyRecipe.RequirementState(requirement.count(), matchingSlots.stream().mapToInt(Integer::intValue).toArray()));
         }

         requirements.sort(
            Comparator.comparingInt(LightningAssemblyRecipe.RequirementState::matchingSlotCount)
               .thenComparing(Comparator.comparingInt(LightningAssemblyRecipe.RequirementState::count).reversed())
         );
         ...
         return !this.allocateRequirement(0, requirements, slotStacks, remainingCounts, slotConsumptions)
            ? Optional.empty()
            : Optional.of(new LightningAssemblyRecipeMatch(slotConsumptions));
      } else {
         return Optional.empty();
      }
   }

   public ItemStack assemble(LightningAssemblyRecipeInput input, RegistryAccess registries) {
      return this.result.m_41777_();
   }
   ...
   public RecipeSerializer<?> m_7707_() {
      return (RecipeSerializer<?>)ModRecipeTypes.LIGHTNING_ASSEMBLY_SERIALIZER.get();
   }

   public RecipeType<?> m_6671_() {
      return (RecipeType<?>)ModRecipeTypes.LIGHTNING_ASSEMBLY_TYPE.get();
   }
   ...
   private boolean allocateRequirement(
      ...
      List<LightningAssemblyRecipe.RequirementState> requirements,
      ...
      if (requirementIndex >= requirements.size()) {
         ...
         LightningAssemblyRecipe.RequirementState requirement = requirements.get(requirementIndex);
   ...
   private boolean allocateAcrossSlots(
      int requirementIndex,
      List<LightningAssemblyRecipe.RequirementState> requirements,
      LightningAssemblyRecipe.RequirementState requirement,
      List<LightningAssemblyRecipeInput.SlotStack> slotStacks,
      int slotCursor,
      int needed,
      int[] remainingCounts,
   ...
   public static final class Serializer implements RecipeSerializer<LightningAssemblyRecipe> {
      public LightningAssemblyRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
         ...
         return new LightningAssemblyRecipe(
            recipeId,
            ...
            RecipeSerializationHelper.enumFromJson(json, "lightningTier", LightningAssemblyRecipe.DEFAULT_LIGHTNING_TIER, LightningKey.Tier.values())
         );
      }

      public LightningAssemblyRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
      ...
      public void toNetwork(FriendlyByteBuf buffer, LightningAssemblyRecipe recipe) {
