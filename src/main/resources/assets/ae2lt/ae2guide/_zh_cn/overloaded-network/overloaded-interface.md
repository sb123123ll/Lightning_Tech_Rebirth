...
|------|------|
| 输出：关 | 不向远程机器输出材料 |
| 输出：自动 | 根据配置槽位自动向远程机器输出材料 |
| 输入：关 | 不从远程机器导入材料 |
| 输入：自动 | 从远程机器持续导入材料到 ME 网络 |
| 输入：弹出 | 远程机器将物品弹入虚拟输入口；接口被动接收 |

当没有开启无线模式时，以上开关会使接口尝试向相邻容器自动推出/回收材料。
...
|-------|--------------|------|
| 普通 | 5 ~ 80 tick | 适合一般场景 |
| 快速（探针） | 1 ~ 40 tick | 使用探针机制提前检测就绪状态 |
...
传输会消耗 ME 网络电力。进行大批量导入或导出时，请确保网络有足够大的 AE 能量缓存；电力不足时，传输会暂停，或只移动当前网络能够支持的数量。
...
* 若过载 ME 接口不进行发配或回收，这往往是网络中能源缓存不足，请尝试增加网络中能源元件的数量

* 特殊情况下，尤其是在更新版本后，过载 ME 接口可能会将物品存入自身缓存内而不返回网络，jade等显示模组不可见，同时也会导致过载 ME 接口失去与网络交互、吞吐能力。因此如果遇到接口无法正常工作的问题，请尝试拆卸重放(拆卸时请注意可能爆掉落物造成严重卡顿)

* 最可能遇见以上问题的情况：接口贴着装有ME磁盘等不可存入网络物品的容器，没有开启无线模式，并且开启了自动回收。(注：编者在做产线时遇到的)

* 同时开启自动输出和自动回收时，不能使用同一个接口将同一类物品从容器a转移到容器b。可能出现的场景有：养蜂时，将蜜脾从蜂箱转入接口，并用该接口将蜜脾送入离心机，这会导致蜂箱内的蜜脾无法正常回收。(依旧做产线遇到)
